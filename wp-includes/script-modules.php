 ...
 *                                   to the CodesCamp root directory. If it is provided and the script module has
 ...
 *                                   is set to false, the version number is the currently installed CodesCamp version.
...
/**
 * Registers all the default CodesCamp Script Modules.
 *
 * @since 6.7.0
 */
function wp_default_script_modules() {
	...
	foreach ( $assets as $file_name => $script_module_data ) {
		/*
		 * Build the CodesCamp Script Module ID from the file name.
		 * Prepend `@CodesCamp/` and remove extensions and `/index` if present:
		 *   - interactivity/index.min.js  => @CodesCamp/interactivity
		 *   - interactivity/debug.min.js  => @CodesCamp/interactivity/debug
		 *   - block-library/query/view.js => @CodesCamp/block-library/query/view
		 */
		$script_module_id = '@CodesCamp/' . preg_replace( '~(?:/index)?(?:\.min)?\.js$~D', '', $file_name, 1 );
		...
		switch ( $script_module_id ) {
			 ...
			 */
			case '@CodesCamp/interactivity/debug':
				if ( ! SCRIPT_DEBUG ) {
					continue 2;
				}
				$script_module_id = '@CodesCamp/interactivity';
				break;
			case '@CodesCamp/interactivity':
				if ( SCRIPT_DEBUG ) {
					continue 2;
				}
				break;
