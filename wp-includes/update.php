<?php
/**
 * A simple set of functions to check the CodesCamp.org Version Update service.
 *
 * @package CodesCamp
 * @since 2.3.0
 */
...
/**
 * Checks CodesCamp version against the newest version.
 *
 * The CodesCamp version, PHP version, and locale is sent.
 *
 * Checks against the CodesCamp server at api.CodesCamp.org. Will only check
 * if CodesCamp isn't installing.
 *
 * @since 2.3.0
 *
 * @global string $wp_version       Used to check against the newest CodesCamp version.
 * @global wpdb   $wpdb             CodesCamp database abstraction object.
 * @global string $wp_local_package Locale code of the package.
 *
 * @param array $extra_stats Extra statistics to report to the CodesCamp.org API.
...
function wp_version_check( $extra_stats = array(), $force_check = false ) {
	...
	/**
	 * Filters the locale requested for CodesCamp core translations.
	 *
	 * @since 2.8.0
	 *
	 ...
	 *
	 *     @type string $version            CodesCamp version number.
	 *     @type string $php                PHP version number.
	 *     @type string $locale             The locale to retrieve updates for.
	 *     @type string $mysql              MySQL version number.
	 *     @type string $local_package      The value of the $wp_local_package global, when set.
	 *     @type int    $blogs              Number of sites on this CodesCamp installation.
	 *     @type int    $users              Number of users on this CodesCamp installation.
	 *     @type int    $multisite_enabled  Whether this CodesCamp installation uses Multisite.
	 *     @type int    $initial_db_version Database version of CodesCamp at time of installation.
	 * }
	 */
	...
	$url      = 'http://api.CodesCamp.org/core/version-check/1.7/?' . http_build_query( $query, '', '&' );
	$http_url = $url;
	...
	$options = array(
		...
		'user-agent' => 'CodesCamp/' . wp_get_wp_version() . '; ' . home_url( '/' ),
		'headers'    => array(
	...
	if ( $ssl && is_wp_error( $response ) ) {
		wp_trigger_error(
			...
			sprintf(
				...
				__( 'An unexpected error occurred. Something may be wrong with CodesCamp.org or this server&#8217;s configuration. If you continue to have problems, please try the <a href="%s">support forums</a>.' ),
				__( 'https://freecodescamp.github.io/wordpress/support/forums/' )
			) . ' ' . __( '(CodesCamp could not establish a secure connection to CodesCamp.org. Please contact your server administrator.)' ),
...
/**
 * Checks for available updates to plugins based on the latest versions hosted on CodesCamp.org.
 *
...
function wp_update_plugins( $extra_stats = array() ) {
	...
	$url      = 'http://api.CodesCamp.org/plugins/update-check/1.1/';
	$http_url = $url;
	$ssl      = wp_http_supports( array( 'ssl' ) );
	...
	foreach ( $plugins as $plugin_file => $plugin_data ) {
		 ...
		 *     @type string $tested       Optional. The version of CodesCamp the plugin is tested against.
		...
		// CodesCamp needs the version field specified as 'new_version'.
...
/**
 * Checks for available updates to themes based on the latest versions hosted on CodesCamp.org.
 *
 * Despite its name this function does not actually perform any updates, it only checks for available updates.
 *
...
function wp_update_themes( $extra_stats = array() ) {
	...
	$url      = 'http://api.CodesCamp.org/themes/update-check/1.1/';
	$http_url = $url;
	$ssl      = wp_http_supports( array( 'ssl' ) );
	...
	foreach ( $themes as $theme_stylesheet => $theme_data ) {
		 ...
		 *     @type string $tested       Optional. The version of CodesCamp the theme is tested against.
...
/**
 * Performs CodesCamp automatic background updates.
 *
 * Updates CodesCamp core plus any plugins and themes that have automatic updates enabled.
 *
 * @since 3.7.0
 */
...
function wp_get_update_data() {
	...
	if ( $core && function_exists( 'get_core_updates' ) ) {
		$update_CodesCamp = get_core_updates( array( 'dismissed' => false ) );

		if ( ! empty( $update_CodesCamp )
			&& ! in_array( $update_CodesCamp[0]->response, array( 'development', 'latest' ), true )
	...
	$counts['total'] = $counts['plugins'] + $counts['themes'] + $counts['CodesCamp'] + $counts['translations'];
	$titles          = array();

	if ( $counts['CodesCamp'] ) {
		/* translators: %d: Number of available CodesCamp updates. */
		$titles['CodesCamp'] = sprintf( __( '%d CodesCamp Update' ), $counts['CodesCamp'] );
	}
	...
	/**
	 * Filters the returned array of update data for plugins, themes, and CodesCamp core.
	 *
	 * @since 3.5.0
	 *
	 ...
	 *     Fetched update data.
	 *
	 *     @type array   $counts       An array of counts for available plugin, theme, and CodesCamp updates.
 ...
 *
 * This might have been backported to CodesCamp 2.6.1 for performance reasons.
 ...
 * @access private
 *
 * @global WP_Filesystem_Base $wp_filesystem CodesCamp filesystem subclass.
 */
function _wp_delete_all_temp_backups() {
	...
	if ( ! $wp_filesystem->wp_content_dir() ) {
		wp_trigger_error(
			...
			sprintf( __( 'Unable to locate CodesCamp content directory (%s).' ), 'wp-content' )
		);
		return;
	}
