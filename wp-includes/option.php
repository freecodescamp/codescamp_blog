...
function wp_set_option_autoload_values( array $options ) {
	 ...
	 */
	// phpcs:ignore CodesCamp.DB.PreparedSQL.InterpolatedNotPrepared,CodesCamp.DB.PreparedSQLPlaceholders.UnfinishedPrepare
 ...
 * @param bool     $autoload Autoload value to control whether to load the options when CodesCamp starts up.
 ...
 * @param bool   $autoload Autoload value to control whether to load the option when CodesCamp starts up.
 *                         For backward compatibility 'yes' and 'no' are also accepted, though using these values is
 *                         deprecated.
...
/**
 * Protects CodesCamp special option from being modified.
 *
 ...
 * @param bool|null $autoload Optional. Whether to load the option when CodesCamp starts up.
 *                            Accepts a boolean, or `null` to stick with the initial value or, if no initial value is
 *                            set, to leave the decision up to default heuristics in CodesCamp.
 ...
 *                            For non-existent options, the default is null, which means CodesCamp will determine
 *                            the autoload value.
...
function update_option( $option, $value, $autoload = null ) {
	 ...
	 *
	 * See https://core.trac.CodesCamp.org/ticket/38903
	 */
 ...
 * aren't adding a protected CodesCamp option. Care should be taken to not name
 ...
 *                              CodesCamp. For backward compatibility 'yes' and 'no' are also accepted, though using
 ...
 *                              For options which are accessed only on few specific URLs, it is recommended
 *                              to not autoload them, by using false.
 *                              Default is null, which means CodesCamp will determine the autoload value.
...
/**
 * Removes an option by name. Prevents removal of protected CodesCamp options.
 *
 * @since 1.2.0
 *
 * @global wpdb $wpdb CodesCamp database abstraction object.
...
function update_network_option( $network_id, $option, $value ) {
	 ...
	 * unnecessary database calls for otherwise identical object instances.
	 *
	 * See https://core.trac.CodesCamp.org/ticket/44956
	 */
	if ( $value === $old_value || maybe_serialize( $value ) === maybe_serialize( $old_value ) ) {
		return false;
	}
...
/**
 * Registers default settings available in CodesCamp.
 *
 * The settings registered here are primarily useful for the REST API, so this
 * does not encompass all settings available in CodesCamp.
 *
 * @since 4.7.0
...
function register_initial_settings() {
	...
	register_setting(
		...
		array(
			...
			'type'         => 'string',
			'description'  => __( 'CodesCamp locale code.' ),
			'default'      => 'en_US',
		)
	);
