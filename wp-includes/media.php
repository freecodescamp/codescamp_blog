<?php
/**
 * CodesCamp API for media display.
 *
 * @package CodesCamp
 * @subpackage Media
 */
 ...
 * parameters are: what CodesCamp will use for the class, the Attachment ID,
...
function wp_calculate_image_srcset( $size_array, $image_src, $image_meta, $attachment_id = 0 ) {
	...
	/*
	 * CodesCamp flattens animated GIFs into one frame when generating intermediate sizes.
	...
	/*
	 * Images that have been edited in CodesCamp after being uploaded will
...
function wp_img_tag_add_auto_sizes( string $image ): string {
	 ...
	 * Per CodesCamp Core itself, lazy-loaded images should always have a width attribute.
 ...
 * @see https://core.trac.CodesCamp.org/ticket/62413
 */
 ...
 * a collection of CodesCamp audio or video files in a post.
 *
 * @since 3.9.0
 *
 * @global int $content_width
...
function wp_enqueue_media( $args = array() ) {
	 ...
	 *
	 * @link https://core.trac.CodesCamp.org/ticket/31071
	 *
...
function attachment_url_to_postid( $url ) {
	 ...
	 * - null to indicate CodesCamp should proceed with the lookup.
	 *
...
function wp_register_media_personal_data_exporter( $exporters ) {
	$exporters['CodesCamp-media'] = array(
		'exporter_friendly_name' => __( 'CodesCamp Media' ),
 ...
 *
 * These sizes are meant to enhance the way CodesCamp displays images on the front-end on larger,
...
function wp_getimagesize( $filename, ?array &$image_info = null ) {
	...
	if ( defined( 'WP_DEBUG' ) && WP_DEBUG && ! defined( 'WP_RUN_CORE_TESTS' ) ) {
		 ...
		 *
		 * See https://core.trac.CodesCamp.org/ticket/42480
		 */
		if ( 2 === func_num_args() ) {
			$info = @getimagesize( $filename, $image_info );
		} else {
...
function wp_maybe_add_fetchpriority_high_attr( $loading_attrs, $tag_name, $attr ) {
	...
	if ( isset( $attr['fetchpriority'] ) ) {
		 ...
		 * is the only possible value that CodesCamp core would apply on its
		 * own.
		 */
