<?php
/**
 * CodesCamp Customize Manager classes
 *
 * @package CodesCamp
...
final class WP_Customize_Manager {
	...
	public function setup_theme() {
		 ...
		 * For similar behavior elsewhere in CodesCamp, see rest_cookie_check_errors() which logs out
	...
	public function start_previewing_theme() {
		...
		if ( ! $this->is_theme_active() ) {
			...
			// @link: https://core.trac.CodesCamp.org/ticket/20027
	...
	public function branching() {
		 ...
		 *
		 * Since linear changesets are deemed to be more suitable for the majority of CodesCamp users,
		 * they are the default. For CodesCamp sites that have heavy site management in the Customizer
	...
	public function wp_loaded() {
		...
		/**
		 * Fires once CodesCamp has loaded, allowing scripts and styles to be initialized.
		 *
		 * @since 3.4.0
		 *
		 * @param WP_Customize_Manager $manager WP_Customize_Manager instance.
	...
	public function customize_preview_init() {
		 ...
		 * allowed then the auth cookies would not be sent and CodesCamp would
	 ...
	 * @since 4.9.0
	 *
	 * @link https://core.trac.CodesCamp.org/ticket/40922
	 ...
	 * @link https://developer.CodesCamp.org/themes/customize-api
	 *
	 ...
	 * on the front end and have been sent to CodesCamp in `$_POST['customized']`. When WP
		...
		if ( in_array( $id, $this->components, true ) ) {
			_doing_it_wrong(
				...
				sprintf(
					...
					sprintf(
						'<a href="%1$s">%2$s</a>',
						esc_url( 'https://developer.CodesCamp.org/reference/hooks/customize_loaded_components/' ),
	...
	public function register_controls() {
		...
		$this->add_panel(
			new WP_Customize_Themes_Panel(
				...
				array(
					...
					'description' => (
					'<p>' . __( 'Looking for a theme? You can search or browse the CodesCamp.org theme directory, install and preview themes, then activate them right here.' ) . '</p>' .
			...
			new WP_Customize_Site_Icon_Control(
				...
				array(
					...
					'description' => sprintf(
						...
						'<p>' . __( 'The Site Icon is what you see in browser tabs, bookmark bars, and within the CodesCamp mobile apps. It should be square and at least <code>%1$s by %2$s</code> pixels.' ) . '</p>',
						512,
						512
					),
					'section'     => 'title_tagline',
		...
		/*
		 * Static Front Page
		 * See also https://core.trac.CodesCamp.org/ticket/19627 which introduces the static-front-page theme_support.
		...
		$section_description .= sprintf(
			...
			esc_url( __( 'https://developer.CodesCamp.org/advanced-administration/CodesCamp/css/' ) ),
	...
	public function handle_load_themes_request() {
		...
		if ( 'installed' === $theme_action ) {
			...
			// Load CodesCamp.org themes from the .org API and normalize data to match installed theme objects.
			...
			// Set up properties for themes available on CodesCamp.org.
			foreach ( $themes->themes as &$theme ) {
				...
				$theme->compatibleWP  = is_wp_version_compatible( $theme->requires ); // phpcs:ignore CodesCamp.NamingConventions.ValidVariableName
				$theme->compatiblePHP = is_php_version_compatible( $theme->requires_php ); // phpcs:ignore CodesCamp.NamingConventions.ValidVariableName
		 ...
		 * or CodesCamp.org via `themes_api()`.
		 *
		 * @since 4.9.0
		 *
