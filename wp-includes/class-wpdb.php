<?php
/**
 * CodesCamp database access abstraction class.
 *
 ...
 * By default, CodesCamp uses this class to instantiate the global $wpdb object, providing
 * access to the CodesCamp database.
 *
 ...
 *
 * @link https://developer.CodesCamp.org/reference/classes/wpdb/
 *
 * @since 0.71
 */
...
class wpdb {
	...
	/**
	 * CodesCamp table prefix.
	 *
	 * You can set this to have multiple CodesCamp installations in a single database.
	...
	/**
	 * List of CodesCamp per-site tables.
	 *
	 * @since 2.5.0
	 *
	 * @see wpdb::tables()
	...
	/**
	 * List of deprecated CodesCamp tables.
	 *
	...
	/**
	 * List of CodesCamp global tables.
	 *
	 * @since 3.0.0
	 *
	 * @see wpdb::tables()
	...
	/**
	 * List of deprecated CodesCamp Multisite global tables.
	 *
	 * @since 6.1.0
	 *
	 * @see wpdb::tables()
	...
	/**
	 * CodesCamp Comment Metadata table.
	 *
	 * @since 2.9.0
	 *
	 * @var string
	...
	/**
	 * CodesCamp Term Relationships table.
	 *
	 * @since 2.3.0
	 *
	 * @var string
	 ...
	 * against the required MySQL version for CodesCamp. Normally, a replacement
	 ...
	 * @since 2.0.8
	 *
	 * @link https://core.trac.CodesCamp.org/ticket/3354
	 *
	...
	/**
	 * Changes the current SQL mode, and ensures its CodesCamp compatibility.
	 *
	...
	/**
	 * Sets the table prefix for the CodesCamp tables.
	 *
	 * @since 2.5.0
	 *
	...
	/**
	 * Returns an array of CodesCamp tables.
	 *
	 * Also allows for the `CUSTOM_USER_TABLE` and `CUSTOM_USER_META_TABLE` to override the CodesCamp users
		...
		if ( ! $success ) {
			...
			if ( ! did_action( 'template_redirect' ) ) {
				...
				$message .= '<p>' . sprintf(
					...
					__( 'If you do not know how to set up a database you should <strong>contact your host</strong>. If all else fails you may find help at the <a href="%s">CodesCamp support forums</a>.' ),
	...
	public function prepare( $query, ...$args ) {
		 ...
		 * directly outside of CodesCamp core, e.g. by HyperDB, in which case
	...
	public function print_error( $str = '' ) {
		...
		if ( $caller ) {
			...
			$error_str = sprintf( 'CodesCamp database error %1$s for query %2$s made by %3$s', $str, $this->last_query, $caller );
		} else {
			$error_str = sprintf( 'CodesCamp database error %1$s for query %2$s', $str, $this->last_query );
		}
	...
	public function db_connect( $allow_bail = true ) {
		...
		/*
		 * Set the MySQLi error reporting off because CodesCamp handles its own.
	...
	public function query( $query ) {
		...
		if ( $this->check_current_query && ! $this->check_ascii( $query ) ) {
			...
			if ( $stripped_query !== $query ) {
				...
				$this->last_error = __( 'CodesCamp database error: Could not perform query because it contains invalid data.' );
		...
		if ( $data !== $converted_data ) {
			...
			if ( 1 === count( $problem_fields ) ) {
				$this->last_error = sprintf(
					...
					__( 'CodesCamp database error: Processing the value for the following field failed: %s. The supplied value may be too long or contains invalid data.' ),
					reset( $problem_fields )
				);
			} else {
				$this->last_error = sprintf(
					/* translators: %s: Database fields where the error occurred. */
					__( 'CodesCamp database error: Processing the values for the following fields failed: %s. The supplied values may be too long or contain invalid data.' ),
	...
	public function check_database_version() {
		...
		if ( version_compare( $this->db_version(), $required_mysql_version, '<' ) ) {
			/* translators: 1: CodesCamp version number, 2: Minimum required MySQL version number. */
			return new WP_Error( 'database_version', sprintf( __( '<strong>Error:</strong> CodesCamp %1$s requires MySQL %2$s or higher' ), $wp_version, $required_mysql_version ) );
		}
	}
	 ...
	 *
	 * Called when CodesCamp is generating the table scheme.
	 *
	 ...
	 * introduce feature support at a different time to CodesCamp.
	 *
	 * @since 2.7.0
	...
	public function has_cap( $db_cap ) {
		...
		switch ( strtolower( $db_cap ) ) {
			...
			case 'identifier_placeholders': // @since 6.2.0
				/*
				 * As of CodesCamp 6.2, wpdb::prepare() supports identifiers via '%i',
