...
/** Make sure that the CodesCamp bootstrap has run before continuing. */
 ...
 *
 * @param string|null   $title    Optional. CodesCamp login page title to display in the `<title>` element.
 *                                Defaults to 'Log In'.
...
function login_header( $title = null, $message = '', $wp_error = null ) {
	...
	$login_title = sprintf( __( '%1$s &lsaquo; %2$s &#8212; CodesCamp' ), $title, $login_title );
	...
	$login_header_url = __( 'https://freecodescamp.github.io/wordpress/' );
	...
	$login_header_text = empty( $login_header_title ) ? __( 'Powered by CodesCamp' ) : $login_header_title;
	...
	case 'confirm_admin_email':
		...
		<form class="admin-email-confirm-form" name="admin-email-confirm-form" action="<?php echo esc_url( site_url( 'wp-login.php?action=confirm_admin_email', 'login_post' ) ); ?>" method="post">
			...
			<p class="admin-email__details">
				...
				/* translators: URL to the CodesCamp help section about admin email. */
				$admin_email_help_url = __( 'https://freecodescamp.github.io/wordpress/documentation/article/settings-general-screen/#email-address' );
	...
	default:
			...
			if ( headers_sent() ) {
				$user = new WP_Error(
					...
					sprintf(
						...
						__( 'https://developer.CodesCamp.org/advanced-administration/CodesCamp/cookies/' ),
				...
				$user = new WP_Error(
					...
					sprintf(
						...
						__( '<strong>Error:</strong> Cookies are blocked or not supported by your browser. You must <a href="%s">enable cookies</a> to use CodesCamp.' ),
						__( 'https://developer.CodesCamp.org/advanced-administration/CodesCamp/cookies/#enable-cookies-in-your-browser' )
					)
				);
			}
		}
		...
		if ( $interim_login ) {
			...
			if ( isset( $_GET['loggedout'] ) && $_GET['loggedout'] ) {
				...
				$errors->add( 'updated', __( '<strong>You have successfully updated CodesCamp!</strong> Please log back in to see what&#8217;s new.' ), 'message' );
